...
public class ImageWriter {
    ...
    public static func generarImagenFirmaHD(
        ...
        let image = renderer.image { ctx in
            ...
            let totalContentHeight = lineH * CGFloat(1 + lineasNombre.count + lineasCargo.count + lineasEmpresa.count + 1 + 1)
            var yStart = (CGFloat(height) - totalContentHeight) / 2 + CGFloat(fontSize) - 1
            ...
            (displayTitulo as NSString).draw(at: CGPoint(x: xTexto, y: currentY), withAttributes: textAttributes)

            currentY += lineH
            for linea in lineasNombre {
                (linea as NSString).draw(at: CGPoint(x: xTexto, y: currentY), withAttributes: textAttributes)
                currentY += lineH
            }
            ...
            (fechaDisplay as NSString).draw(at: CGPoint(x: xTexto, y: currentY), withAttributes: textAttributes)
            currentY += lineH

            ("Firmado con ACJ Signature" as NSString).draw(at: CGPoint(x: xTexto, y: currentY), withAttributes: textAttributes)
        }
        return image
    }
